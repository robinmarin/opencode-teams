...
export function createEventHandler(
  client: Client,
  getLogger: GetLogger,
  { skipRecovery = false }: { skipRecovery?: boolean } = {},
  ...
  return async ({ event }) => {
    ...
    return withSessionQueue(sessionID, () =>
      handleEvent(client, getLogger, event, sessionID),
    );
  };
}
...
async function handleEvent(
  ...
  if (memberName === LEAD_MEMBER_NAME) {
    ...
    if (busyMembers.length > 0) {
      ...
      try {
        ...
        log.error(
          "messaging",
          "promptAsync threw for lead (busy-members notice)",
          {
            leadSessionId: team.leadSessionId,
            error: String(err),
          },
  ...
  if (member === undefined) {
    log.warn(
      "messaging",
      "member found by session index but missing from team config",
      {
        memberName,
        sessionId: sessionID,
      },
  ...
  if (event.type === "session.status") {
    ...
    if (status.type === "idle") {
      log.debug(
        "messaging",
        "session.status idle ignored; waiting for session.idle",
        {
          memberName,
          sessionId: sessionID,
        },
  ...
  if (wasBusy) {
    ...
    if (elapsed < COOLDOWN_MS) {
      ...
      try {
        ...
        log.debug("messaging", "status event posted to channel", {
          memberName,
        });
      } catch (err) {
  ...
  // Always send a plain-text team status summary to the lead so they have
  // an up-to-date view of the team, even if the member was already ready.
