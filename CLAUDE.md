...
- **`tool`** — 19 tools built by `createTools(client)` in `src/tools.ts`
...
All 19 tools use `tool.schema` (the plugin's bundled Zod instance, v4.1.8) rather than importing `zod` directly. Importing from the root `zod` package causes a version-mismatch type error because the plugin vendors its own copy.
